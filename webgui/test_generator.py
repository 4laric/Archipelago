...
class TestHostingPageAdvertisesHonestly:
    ...
    🛑 THESE TESTS MOVED FROM `/` TO `/hosting` AND THAT MATTERS MORE THAN IT LOOKS. `/` is the
    landing page now, and two of the five assert an ABSENCE -- so pointed at `/` they would have
    gone green against a page that could never have contained the string in the first place. A
    test that passes because it is aimed at the wrong page is worse than a deleted one: it reports
    coverage of a surface nobody is checking.
    """
    ...
    def test_offers_the_wizard_when_the_tooling_is_deployed(self, tmp_path, monkeypatch):
        ...
        html = self._client(monkeypatch, er_dir=str(tmp_path)).get("/hosting").data.decode()
        assert "/er/" in html and "options wizard" in html.lower()

    def test_stays_quiet_about_the_wizard_when_it_is_not(self, monkeypatch):
        html = self._client(monkeypatch, er_dir="").get("/hosting").data.decode()
        assert "/er/" not in html

    def test_does_not_claim_generation_without_an_ap_tree(self, tmp_path, monkeypatch):
        (tmp_path / "wizard.html").write_text("w", encoding="utf-8")
        html = self._client(monkeypatch, er_dir=str(tmp_path), ap_root=str(tmp_path)).get("/hosting").data.decode()
